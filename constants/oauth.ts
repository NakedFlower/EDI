...
const bundleId = "space.manus.ideavault.t20260307233847";
...
/**
 * Detect if running inside Expo Go (not a standalone/dev build).
 */
function isExpoGo(): boolean {
  // In Expo Go, executionEnvironment is 'storeClient'
  // In dev builds, it's 'standalone' or 'bare'
  const execEnv = (Constants as any).executionEnvironment;
  return execEnv === "storeClient";
}

/**
 * Get the redirect URI for OAuth callback.
 * - Web: uses API server callback endpoint (https)
 * - Native (Expo Go): uses server's /api/oauth/native-callback (https) since exp:// is not allowed
 * - Native (built app): uses manus* deep link scheme
 */
export const getRedirectUri = () => {
  ...
  // The server will redirect back to the app via exp:// deep link after auth
  if (isExpoGo()) {
    return `${getApiBaseUrl()}/api/oauth/native-callback`;
  }

  // In standalone/dev builds, use the manus* scheme directly
